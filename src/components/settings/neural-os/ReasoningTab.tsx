...
export function ReasoningTab() {
  ...
  return (
    <div className="space-y-6">
      <NeuralOsHeader
        title="Reasoning Layer | Neural OS"
        description="The Reasoning layer diagnoses issues, scopes work, and generates estimates. Multi-agent system with semantic SWI matching."
      />

      {/* Max persona block (Max stays — UX persona, not brand) */}
      <div className="flex items-start justify-between gap-4">
        <div className="flex gap-3 items-end">
          <img
            src={aiAgentAvatar}
            alt="Max — Neural OS reasoning agent"
            className="w-[96px] h-[96px] rounded-xl object-cover shrink-0"
            loading="lazy"
            width={96}
            height={96}
            decoding="async"
          />
          <div>
            <h3 className="text-base font-semibold text-foreground">Hi, I'm Max!</h3>
            <p className="text-xs text-muted-foreground mt-1 max-w-2xl">
              I run the Reasoning layer. AI prompts define how each of my agents analyzes incidents and
              generates responses. Templates use Jinja2 and validate output against JSON schemas. Changes
        ...
        <div className="flex gap-3">
          ...
          <div className="text-sm">
            <p className="text-blue-800 dark:text-blue-200 font-medium">
              Neural OS uses a multi-agent system to analyze service tickets, match SWIs, and generate
              professional assessments.
            </p>
            <p className="text-blue-600 dark:text-blue-400 text-xs mt-1">
              Note: The AI engine is powered by Noch Power. You can test and execute prompts directly
              from this tab.
            </p>
          </div>
        </div>
      ...
      {/* Agent Cards */}
      <div>
        <h3 className="text-sm font-bold text-foreground mb-4 uppercase tracking-wide">Agent Prompts</h3>
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            {agents.length === 0 && (
              <p className="text-center text-sm text-muted-foreground py-8">
                No agent prompts configured yet.
              </p>
            )}
          </div>
        )}
