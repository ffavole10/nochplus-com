...
// SVG internal coordinates for charger icons
...
// Map constants
const CENTER: [number, number] = [36.1215, -115.1739];
const SVG_BOUNDS: L.LatLngBoundsExpression = [[36.1200, -115.1760], [36.1230, -115.1718]];
const ZOOM_THRESHOLD = 13;
const TILE_URL = "https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png";

// Sites for far-zoom markers
const SITES = [
  {
...
/** Tracks zoom level and notifies parent */
...
export function MonitoringMapView({ filter, onSelectCharger }: Props) {
  ...
  const showSVG = currentZoom >= ZOOM_THRESHOLD;
    ...
    <div className="relative w-full" style={{ height: 'calc(100vh - 280px)', minHeight: 480 }}>
      ...
      <div className="absolute inset-0 rounded-lg border border-border overflow-hidden">
        ...
        >
          ...
          {!showSVG && SITES.map((site) => (
            <CircleMarker
              key={site.name}
              center={site.position}
              radius={12}
              pathOptions={{
                color: site.hasCritical ? '#D93025' : '#1B8A7A',
                fillColor: site.hasCritical ? '#D93025' : '#1B8A7A',
                fillOpacity: 0.7,
                weight: 2,
              }}
            >
              <Popup>
                <div className="text-xs font-medium">
                  ...
                  <div className="mt-1">{site.chargers} chargers · Fleet CVS {site.cvs} · {site.critical} Critical</div>
                </div>
              </Popup>
            </CircleMarker>
          ))}
